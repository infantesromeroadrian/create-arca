---
name: langchain-rag
description: "RAG with LangChain: document loaders, RecursiveCharacterTextSplitter, embeddings, vector stores (Chroma, FAISS, Pinecone). Use for LangChain RAG implementation. For framework-agnostic RAG design, use rag-systems instead."
---

<arca-preference>
Dentro de ARCA, sustituir `ChatOpenAI(model="gpt-4.1")` por `ChatAnthropic(model="claude-sonnet-4-6")` en la fase de generación. Los embeddings (`OpenAIEmbeddings` / `text-embedding-3-small`) se mantienen en los ejemplos porque Anthropic no provee embeddings nativos; alternativas validadas para ARCA son Voyage AI o `BAAI/bge-large-en-v1.5` local (ver skill `rag-systems`).
</arca-preference>
