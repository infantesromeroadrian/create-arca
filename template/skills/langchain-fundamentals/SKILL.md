...
description: "LangChain BASICS: create_agent(), @tool decorator, chains, prompt templates. Use when building from scratch or learning core concepts. For production patterns, use langchain instead."
---
...
> **ARCA preference:** al calcar estos ejemplos en código ARCA, sustituir `model="gpt-4.1"` por `"anthropic:claude-sonnet-4-6"` (default ARCA) o `"anthropic:claude-opus-4-8"` (high-stakes). Los ejemplos respetan la convención mixta de la doc oficial de LangChain v1 a propósito — facilita comparar con otras skills del cluster (langchain, langchain-middleware, langgraph).
...
|-----------|---------|---------|
| `model` | LLM to use | `"anthropic:claude-sonnet-4-6"` or model instance |
...
`create_agent` accepts model strings (`"anthropic:claude-sonnet-4-6"`, `"openai:gpt-4.1"`) or model instances for custom settings:

```python
from langchain_anthropic import ChatAnthropic
agent = create_agent(model=ChatAnthropic(model="claude-sonnet-4-6", temperature=0), tools=[...])
```
</model_config>
