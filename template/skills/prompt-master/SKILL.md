...
- Be explicit and specific — Claude 4.x follows instructions literally. Opus 4.8 especially: it does exactly what you say, nothing more. Missing context = narrow literal output, not a smart guess.
...
- For complex or multi-step tasks on Opus 4.8: front-load everything in one turn — intent, constraints, acceptance criteria, relevant files. Every extra back-and-forth turn adds reasoning overhead and token cost.
- Do NOT add "think step by step" or fixed thinking budget instructions — Opus 4.8 uses adaptive thinking and calibrates depth automatically. To influence depth: "Think carefully before responding" (more) or "Prioritize responding quickly" (less).
- Use Template M for agentic or multi-step tasks on Opus 4.8.
...
- Opus 4.8 default in Claude Code is xhigh effort — do NOT specify effort level in prompts, it's already set
- Opus 4.8 is more literal than 4.6 — vague first turns produce narrower results. Front-load everything: intent, file scope, constraints, acceptance criteria, session strategy.
...
**Computer-Use / Browser Agents** (Perplexity Comet/Computer, OpenAI Atlas, Claude in Chrome, your-mobile-stack Agents)
