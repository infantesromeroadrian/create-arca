---
name: transilience-reconnaissance
...
- `transilience-injection` - Hand off discovered parameters and endpoints for injection testing.
- `transilience-api-security` - Hand off discovered REST/GraphQL/SOAP/WebSocket endpoints for deeper API testing.

OSINT footprinting (repository enumeration, secret scanning, git history analysis, employee/email footprinting) is folded into this skill rather than delegated: run it inline alongside subdomain enumeration. Enumerate public repos (`github`/`gitlab` org search), scan checkouts for hardcoded secrets (`trufflehog`, `gitleaks`), and walk git history for removed-but-recoverable credentials.

## Rules

1. Passive discovery before active scanning
2. Run the inline OSINT footprinting (repo/secret/history scanning) in parallel during Phase 2
...
11. **Focused AD port scan for Windows targets**: when initial fingerprinting shows a Windows DC archetype (any of 53/135/139/445/389 open), skip `-p-` and run a focused scan over the 13 AD-relevant ports first — it finishes in seconds and covers everything that matters.
    ...
    ```
    Ports rationale: 53 DNS, 88 Kerberos, 135 RPC, 139/445 SMB, 389/636 LDAP/LDAPS, 464 kpasswd, 593 RPC-over-HTTPS, 3268/3269 GC/GC-LDAPS, 5985 WinRM (HTTP), 5986 WinRM (HTTPS — cert auth), 9389 AD Web Services. Always probe BOTH 5985 and 5986 — when 5985 is filtered, 5986 with client-cert auth is a common foothold path. WinRM cert-based auth: when only 5986 (HTTPS) answers and password auth fails, look for a leaked client certificate (PFX/PEM in shares, web roots, or config dumps) and authenticate with `evil-winrm -S -i TARGET -c cert.pem -k key.pem` (or map the cert to a user via PowerShell remoting over the cert-mapped endpoint). Only fall back to `-p-` if (a) no flag-yielding service surfaces in the focused scan, or (b) you suspect a non-standard app on a high port (custom web service, RDP-on-non-3389, etc.). Don't burn 30 minutes on full TCP sweeps when the AD archetype is obvious.
