---
description: AIDesigner MCP workflow for design generation — uses `aidesigner` MCP server (whoami, get_credit_status, generate_design, refine_design). Spends credits only when user explicitly invokes; pre-flight reads DESIGN.md and theme tokens.
---
...
<!-- AUTO-GENERATED from packages/aidesigner-agent-skills/templates/claude-command.md — do not edit directly.
     ...
     P1-G2 fix 2026-05-16: frontmatter moved to line 1 (above this comment) so the
     Claude Code runtime can route this command. The upstream regen step will
     drop this frontmatter; re-add it from CHANGELOG[Unreleased] post-upgrade. -->
