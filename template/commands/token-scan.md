---
description: Meme coin and token security scan — checks for rug pull vectors (hidden mint, honeypot, fee manipulation, LP lock bypass, authority retention, bonding curve exploits, fake renounce, sandwich amplification). Delegates to the meme-coin-audit skill via @bug-bounty-hunter for the manual 8-class audit. Usage: /token-scan <contract_path_or_dir> [--chain solana]
---
...
## Step 1: Delegate To The Audit Skill

Hand the target path to `@bug-bounty-hunter` loading the `meme-coin-audit` skill. The skill drives the full 8-class rug-pull audit (EVM Solidity + Solana Rust/Anchor), produces a risk score + verdict, and reports findings with file:line evidence.

The manual grep passes in Steps 2-9 below are the canonical checklist the skill follows — run them directly when you want a fast first-pass triage before invoking the full audit.
...
The `meme-coin-audit` skill produces:
